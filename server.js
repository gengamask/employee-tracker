...
const company = async () => {
    ...
    if(comp.home === 'View All Roles'){
        db.query('SELECT roles.id, title, department.name AS department, salary FROM roles JOIN department ON roles.department_id = department.id', function(err, result) {
            console.error(err)
            console.table(result);
            company();
        })
    ...
    if(comp.home === 'View All Employees'){
        db.query("SELECT employee.id, employee.first_name, employee.last_name, roles.title AS title, department.name AS department, roles.salary AS salary, CONCAT(m.first_name,',', m.last_name ) AS manager FROM employee LEFT JOIN employee m ON m.id = employee.manager_id JOIN roles ON employee.role_id = roles.id JOIN department ON roles.department_id = department.id", function(err, result) {
        ...
        iq.prompt([
            {
                type: "input",
                name: "adddept",
                message: "What is the name of the departmnt that you would like to add?"
            }
        ])
